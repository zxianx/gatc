...
type GormOfficialTokens struct {
	...
	LastUsedAt        time.Time `gorm:"column:last_used_at" json:"last_used_at,omitempty"`                         //'最后使用时间'
	...
	ExternalSyncAt    time.Time `gorm:"column:external_sync_at" json:"external_sync_at,omitempty"`       //'最后外部同步时间'
	...
	TokenExpiresAt    time.Time `gorm:"column:token_expires_at" json:"token_expires_at,omitempty"`       //'OAuth2令牌过期时间'
